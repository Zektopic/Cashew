  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < filteredSpotsFixedX.length; i++) {
      ...
      for (int j = 0; j < filteredSpotsFixedX[i].length; j++) {
        initialSpots.add(FlSpot(j.toDouble(), minY));
      }
          ...
          aboveBarData: widget.minY >= -1e-14
              ...
              : BarAreaData(
                  ...
                  gradient: LinearGradient(
                    colors: [
                      i == 0
                          ? (widget.lineColors?[i] ?? widget.color).withAlpha(
                              100,
                            )
                          : (widget.lineColors?[i] ?? widget.color).withAlpha(
                              1,
                ...
                getTooltipItems: (List<LineBarSpot> lineBarsSpot) {
                  return lineBarsSpot.asMap().entries.map((entry) {
                    ...
                    if (dateRange != null && widget.showDateOnHover) {
                      String startDateString = getWordedDateShort(
                        dateRange.start,
                      );
              ...
              leftTitles: AxisTitles(
                sideTitles: SideTitles(
                  ...
                  getTitlesWidget: (value, titleMeta) {
                    bool show = value == 0 ||
                        (value <= widget.maxY && value > widget.minY);
                    if (!show) {
                      return SizedBox.shrink();
                    }
                    return Padding(
