...
- 2026-03-26: Iterative Enhancement - Audited CSV export logic. Discovered and mitigated a CSV Injection (Formula Injection) vulnerability in `budget/lib/widgets/exportCSV.dart`. All user-generated fields are now sanitized by prefixing formula characters with a single quote to prevent arbitrary execution in spreadsheet software.\n**Next Planned Step:** Consider adding an animation for the auto-collapse transition to provide visual feedback before the balance hides, or audit the obfuscation logic across exported PDF reports.
...
- **2025-03-29 - CSV Injection in Export Functions:**
  - **Vulnerability/Gap:** User input from transaction fields (e.g., titles, notes, categories) was being directly exported to CSV without escaping. This creates a CSV Injection (or Formula Injection) vulnerability where malicious strings starting with `=`, `+`, `-`, or `@` could execute arbitrary formulas or commands when opened in spreadsheet software like Excel or Google Sheets.
  - **Learning:** Applications that generate structured exports (like CSVs) from user-controlled data must treat that data as potentially malicious output.
  - **Prevention:** Always sanitize user data upon export. In CSVs, prefix cells starting with formula trigger characters with a single quote (`'`) to force the spreadsheet application to interpret the cell strictly as text, neutralizing any formula execution.
